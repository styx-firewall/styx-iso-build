...
echo "deb [arch=amd64 signed-by=/usr/share/keyrings/styx-firewall-keyring.gpg] https://styx-firewall.github.io/styx-repo trixie __STYX_BRANCH__" | tee /etc/apt/sources.list.d/styx.list
...
# Plus others for 'ready to test' configurations like s2s
#
...
openssl req -x509 -newkey rsa:4096 -sha256 -days 3650 -nodes \
  ...
  -subj "/C=XX/ST=Autogenerated/L=Autogenerated/O=Styx Example Self-Signed/OU=Autogenerated Certificate/CN=styx.local" \
  -addext "basicConstraints=CA:FALSE" \
  -addext "subjectAltName=DNS:styx.local,$IPS" \
  -addext "extendedKeyUsage=serverAuth,clientAuth,1.3.6.1.5.5.8.2.2" \
...
openssl genrsa -out /etc/ssl/local-private/ca-key.pem 4096
chmod 600 /etc/ssl/local-private/ca-key.pem

# 2) Generate self-signed CA certificate (CA:TRUE)
openssl req -x509 -new -key /etc/ssl/local-private/ca-key.pem \
  -sha256 -days 7300 -nodes \
  -out /etc/ssl/local-certs/ca-cert.pem \
  -subj "/C=XX/ST=Autogenerated/L=Autogenerated/O=Styx Firewall/OU=Certificate Authority/CN=Styx Root CA" \
  -addext "basicConstraints=critical,CA:TRUE,pathlen:0" \
  -addext "keyUsage=critical,keyCertSign,cRLSign,digitalSignature"

chmod 644 /etc/ssl/local-certs/ca-cert.pem

# 3) Generate CSR + key for the CA-signed server certificate
IPS=$(hostname -I | awk '{for(i=1;i<=NF;i++) printf "IP:"$i","}' | sed 's/,$//')
openssl req -new -newkey rsa:4096 -nodes \
  -keyout /etc/ssl/local-private/https-server-key.pem \
  -out /tmp/https-server.csr \
  -subj "/C=XX/ST=Autogenerated/L=Autogenerated/O=Styx Firewall/OU=Gateway/CN=styx.local"

chmod 600 /etc/ssl/local-private/https-server-key.pem

# 4) Sign the CSR with the CA
openssl x509 -req -in /tmp/https-server.csr \
  -CA /etc/ssl/local-certs/ca-cert.pem \
  -CAkey /etc/ssl/local-private/ca-key.pem \
  -CAcreateserial -days 3650 -sha256 \
  -out /etc/ssl/local-certs/https-server-cert.pem \
  -extfile <(cat <<EOF
basicConstraints=CA:FALSE
subjectAltName=DNS:styx.local,$IPS
extendedKeyUsage=serverAuth,clientAuth,1.3.6.1.5.5.8.2.2
keyUsage=digitalSignature,keyEncipherment
EOF
)

chmod 644 /etc/ssl/local-certs/https-server-cert.pem
rm -f /tmp/https-server.csr

# 5) Install CA certificate into system trust store
cp /etc/ssl/local-certs/ca-cert.pem /usr/local/share/ca-certificates/styx-root-ca.crt
update-ca-certificates 2>/dev/null || true

echo "  -> CA certificate: /etc/ssl/local-certs/ca-cert.pem"
echo "  -> CA key: /etc/ssl/local-private/ca-key.pem"
...
gen_s2s_cert() {
  local peer="$1"
  local cn="styx-s2s-${peer}.local"
  local key="/etc/ssl/local-private/https-s2s-${peer}-key.pem"
  local crs="/tmp/https-s2s-${peer}.csr"
  local cert="/etc/ssl/local-certs/https-s2s-${peer}-cert.pem"

  openssl req -new -newkey rsa:4096 -nodes \
    -keyout "$key" \
    -out "$crs" \
    -subj "/C=XX/ST=Autogenerated/L=Autogenerated/O=Styx Firewall/OU=S2S Testing/CN=${cn}"

  chmod 600 "$key"

  openssl x509 -req -in "$crs" \
    -CA /etc/ssl/local-certs/ca-cert.pem \
    -CAkey /etc/ssl/local-private/ca-key.pem \
    -CAcreateserial -days 3650 -sha256 \
    -out "$cert" \
...
EOF
  ...
  chmod 644 "$cert"
  rm -f "$crs"
  echo "  -> S2S peer ${peer}: ${cert}"
  echo "  -> S2S peer ${peer} key: ${key}"
}
